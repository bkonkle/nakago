...
use sea_orm::{
    DatabaseBackend, DatabaseConnection, MockDatabase, MockExecResult, Transaction, Value,
};
...
async fn test_shows_service_get() -> Result<()> {
    ...
    let i = setup(
        MockDatabase::new(DatabaseBackend::Postgres).append_query_results(vec![vec![show.clone()]]),
    )
    .await?;
