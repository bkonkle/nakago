...
Nakago's `Inject` framework is built with [Tokio](https://tokio.rs/) and [Shared Futures](https://docs.rs/futures/latest/futures/future/struct.Shared.html), allowing multiple threads to request and await the same dependency and use an Arc to hold on to it across await points without worrying about lifetimes.
...
By default, the injection framework works directly with the `TypeId` identifiers that are automatically generated by the [any](https://doc.rust-lang.org/std/any/index.html) package. If you need muultiple instances of a particular type, though, you can use a `Tag` to differentiate between them.
...
```rust
use async_trait::async_trait;
use nakago::{inject, Provider, Inject};
use sqlx::{Pool, Postgres};
